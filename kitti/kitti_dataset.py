...
if __name__=='__main__':
    kitti_dataset = KittiDataset(root_dir='.', split='train')
    img = kitti_dataset.get_image(0)
    img_shape = kitti_dataset.get_image_shape(0)
    pt = kitti_dataset.get_lidar(0)
    calib = kitti_dataset.get_calib(0)
    label = kitti_dataset.get_label(0)
    road = kitti_dataset.get_road_plane(0)
    a = 1