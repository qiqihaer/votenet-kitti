...
class GTDatabaseGenerator(KittiDataset):
    ...
    def generate_gt_database(self):
        ...
        for idx, sample_id in enumerate(self.image_idx_list):
            ...
            # --------wq:20191217
            gt_boxes3d_corners = boxes3d_to_corners3d(gt_boxes3d)
            for k in range(gt_boxes3d_corners.__len__()):
                cur_pts, pts_in_3d_box_inds = extract_pc_in_box3d(pts_rect, gt_boxes3d_corners[k])
                # pt_mask_flag = (boxes_pts_mask_list[k].numpy() == 1)
                # cur_pts = pts_rect[pt_mask_flag].astype(np.float32)
                cur_pts_intensity = pts_intensity[pts_in_3d_box_inds].astype(np.float32)
