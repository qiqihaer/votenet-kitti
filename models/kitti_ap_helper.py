...
def parse_predictions(end_points, config_dict):
    ...
    for i in range(bsize):
        for j in range(num_proposal):
            ...
            corners_3d_upright_camera = get_3d_box(box_size, heading_angle, pred_center[i, j, :])
...
def parse_groundtruths(end_points, config_dict):
    ...
    center_label = center_label.detach().cpu().numpy()
    for i in range(bsize):
        for j in range(K2):
            ...
            corners_3d_upright_camera = get_3d_box(box_size, heading_angle, center_label[i, j, :])
